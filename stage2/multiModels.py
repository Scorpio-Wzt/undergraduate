...
class BERT_BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.special_tokens_yi = ['[TAB]', '[LINE]',
                                  '[EQU]', '[URL]', '[NUM]',
                                  '[SPE]', '<sep>', '[q]']
        self.special_tokens_dict_yi = {
            'additional_special_tokens': self.special_tokens_yi}

        self.longtokenizer = AutoTokenizer.from_pretrained(
            './Lawformer/')
        self.longtokenizer.add_special_tokens(self.special_tokens_dict_yi)

        self.longformerconfig = AutoConfig.from_pretrained(
            './Lawformer/')
        self.longformerconfig.attention_mode = 'sliding_chunks'
        self.longformerconfig.attention_window = [
            8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8]
        self.attentionwindow = self.longformerconfig.attention_window[0]

        self.longformer = AutoModel.from_pretrained(
            './Lawformer/', config=self.longformerconfig)
        ...
        self.span_loss_candidates = [
            "all", "pred_and_gold", "pred_gold_random", "gold"][0]
    ...
    def compute_loss(self, start_logits, end_logits, span_logits,
        ...
        if self.span_loss_candidates == "all":
            # naive mask
            float_match_label_mask = match_label_mask.view(
                batch_size, -1).float()
        else:
            ...
            if self.span_loss_candidates == "gold":
                ...
                random_matrix = torch.empty(
                    batch_size, seq_len, seq_len).uniform_(0, 1)
        ...
        end_loss = self.bce_loss(
            end_logits.view(-1), end_labels.view(-1).float())
        end_loss = (end_loss * end_float_label_mask).sum() / \
            end_float_label_mask.sum()
        match_loss = self.bce_loss(span_logits.view(
            batch_size, -1), match_labels.view(batch_size, -1).float())
    ...
    def bert_emb_for_task1(self, para_tokens_list):
        ...
        question_tokens = '[q]'
        question_tokens_cls_sep = self.longtokenizer.cls_token + \
            ' ' + question_tokens + ' ' + self.longtokenizer.sep_token
        question_ids = self.longtokenizer.convert_tokens_to_ids(
            question_tokens_cls_sep.split(' '))
        question_length = len(question_ids)

        sent_tokens_list = [sent for para in para_tokens_list for sent in para]
        sent_length_list = [len(sent.split(' '))
                            for para in para_tokens_list for sent in para]
        passage_tokens = ' '.join(sent_tokens_list)
        passage_tokens_cls_sep = self.longtokenizer.cls_token + \
            ' ' + passage_tokens + ' ' + self.longtokenizer.sep_token

        sent_ids = self.longtokenizer.convert_tokens_to_ids(
            passage_tokens_cls_sep.split(' '))  # !

        question_sents_ids = [question_ids + sent_ids]

        qs_ids_padding_list, qs_mask_list, max_len = self.padding_and_mask_with_return(
            question_sents_ids)
        qs_ids_padding_tensor = torch.tensor(qs_ids_padding_list).cuda()
        qs_mask_tensor = torch.tensor(qs_mask_list).cuda()

        _, global_att_mask = self.padding_and_mask_to_max_lenth(
            [question_ids], max_len)
        ...
        last_hidden_state_for_s = last_hidden_state[:,
                                                    (question_length + 1):-1, :]
    ...
    def bert_emb_for_task2(self, para_tokens_list, argument_para_tokens_list):
        ...
        argument_tokens_cls_sep = self.longtokenizer.cls_token + \
            ' '+argument_tokens+' '+self.longtokenizer.sep_token
        argument_ids = self.longtokenizer.convert_tokens_to_ids(
            argument_tokens_cls_sep.split(' '))
        ...
        sent_length_list = [len(sent.split(' '))
                            for para in para_tokens_list for sent in para]
        passage_tokens = ' '.join(sent_tokens_list)
        passage_tokens_cls_sep = self.longtokenizer.sep_token + \
            ' '+passage_tokens+' '+self.longtokenizer.sep_token
        sent_ids = self.longtokenizer.convert_tokens_to_ids(
            passage_tokens_cls_sep.split(' '))
    ...
    def am_tagging_span_for_task1(self, para_tokens_list, mode):
        ...
        para_lstm_out_padded, _ = pad_packed_sequence(
            para_lstm_out_packed, batch_first=True)
    ...
    def am_tagging_with_task1_for_task2(self, rev_para_tokens_list, rep_para_tokens_list, arg_list_from_task1,
        ...
        para_lstm_out = lstm_out_cat
        batch_size, seq_len, hid_size = para_lstm_out.size()
        start_logits = self.start_outputs(para_lstm_out).squeeze(-1)
        end_logits = self.end_outputs(para_lstm_out).squeeze(-1)
        start_extend = para_lstm_out.unsqueeze(2).expand(-1, -1, seq_len, -1)
        end_extend = para_lstm_out.unsqueeze(1).expand(-1, seq_len, -1, -1)
        span_matrix = torch.cat([start_extend, end_extend], 3)
    ...
    def forward(self, video_feature_list_o, para_tokens_list_o, para_tokens_list_for_2_o, rr_arg_pair_list_o,
                match_labels_o, start_labels_o, end_labels_o, tag_list_o):
        total_loss_all = 0
        for video_feature, para_tokens, para_tokens_2, rr_arg_pair, tag, start_labels, end_labels, match_labels in zip(video_feature_list_o, para_tokens_list_o, para_tokens_list_for_2_o, rr_arg_pair_list_o, tag_list_o, start_labels_o, end_labels_o, match_labels_o):
            video_feature_list = [video_feature]
            ...
            if tag == "task1_review" or tag == "task1_reply":
                start_logits, end_logits, span_logits = self.am_tagging_span_for_task1(
                    para_tokens_list, mode="train")
            ...
            start_loss, end_loss, match_loss = self.compute_loss(start_logits,
                                                                 ...
                                                                 end_labels.expand(
                                                                     1, -1).cuda(),
                                                                 match_labels.expand(1, -1, -1).cuda())
            total_loss = self.weight_start * start_loss + \
                self.weight_end * end_loss + self.weight_span * match_loss
